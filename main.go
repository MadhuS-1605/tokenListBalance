...
import (
	...
	tokenListBalance "tokenListBalance/testbuild"
...
func TokenBal(tokenaddress string, wg *sync.WaitGroup, tokenAddress common.Address, tokens Tokens, i int, client *ethclient.Client) {
	...
	instance, err := tokenListBalance.NewToken(tokenAddress, client)
	defer wg.Done()
	if err != nil {
		log.Fatal(err)
	}
...
func main() {
	...
	jsonFile, err := os.Open("tokenlist.json")
